...
from app.db.Database.database import get_settings
...
async def test_initialize_register(client):
    ...
    response = await client.post("/Register", json={"email": email})
...
async def test_request_otp(client):
    ...
    response = await client.post("/testOTP", json={"email": email})
    assert response.status_code == status.HTTP_201_CREATED
...
async def test_verify_otp(client):
    ...
    response = await client.post("/VerifyOTP", json={"email": email, "otp": otp})
